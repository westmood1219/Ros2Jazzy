...
有一种很容易误判的问题：RViz 里 LaserScan 看起来像探测到一面近墙，但 Gazebo 世界里并没有那面墙。

这次的典型症状是：

- 前进时，一半扫描正常，另一半出现约 `0.3m` 的近距离"墙"
- 停车后，近墙方向会翻转
- 后退时，现象又随着运动方向变化
- 静态看 TF 和墙体位置都不像明显错误

这种情况要优先怀疑：**雷达扫描平面不是水平的，而是在车体俯仰时扫到了地面**。

排查顺序：

1. 先确认 `/scan` 的近距离值是否随加速、减速、停车状态变化。如果会随动态状态翻转，它通常不是固定墙体。
2. 看机器人是否三点或四点稳定接地。差速车如果只有左右两个轮子支撑，车体很容易绕轮轴俯仰。
3. 计算每个接触件的世界高度，尤其是万向轮、支撑球、轮子底部：

```text
link 世界高度 + joint z - collision 半径/半高 = 接触件底部高度
```

4. 对比雷达高度和假墙距离。雷达高度约 `0.12m` 时，轻微俯仰就可能让光束在几十厘米外打到地面。
5. 如果接地点已经贴地但问题仍偶发，继续检查支撑多边形是否覆盖质心前后方向。质心落在驱动轮轴线附近时，前进、后退、刹车仍可能让车体跷动。
6. 修复几何后重启仿真，用同样的键盘动作复现验证。

这类问题的关键是：**RViz 显示的是 scan 点，不负责判断点来自墙还是地面**。只要激光束打到地面，RViz 也会忠实显示成一圈障碍。
